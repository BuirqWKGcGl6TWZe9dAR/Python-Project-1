...
class BankAccount:
    def __init__(self, name="John", email="john@gmail.com", initial_balance=0):
        if not isinstance(initial_balance, (int, float)) or initial_balance < 0:
            ...
            initial_balance = 0  # fallback to 0
...
class SavingsAccount(BankAccount):
    ...
    def withdraw(self, amount):
        if self.balance - amount < self.MIN_BALANCE:
            print(f"Cannot withdraw. Minimum balance of Rs.{self.MIN_BALANCE} must be maintained.")
            return
...
class StudentAccount(BankAccount):
    def withdraw(self, amount):
        if self.balance - amount < 100:
            print("A minimum balance of Rs.100 must remain in Student Account!")
            return
        super().withdraw(amount)
